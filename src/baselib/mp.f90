...
!      PRIVATE
      PUBLIC :: mp_start, mp_end, mp_env, mp_group, mp_cart_create,   &
        ...
        mp_barrier, mp_report, mp_comm_split
!
      ...
      INTEGER, PRIVATE, PARAMETER :: mp_msgsiz_max = 500000000
...
!
!
      SUBROUTINE mp_put_cm(msg_dest, msg_sour, mpime, sour, dest, ip, gid)
        COMPLEX (dbl) :: msg_dest(:,:), msg_sour(:,:)
        INTEGER, INTENT(IN) :: dest, sour, ip, mpime
        INTEGER, OPTIONAL, INTENT(IN) :: gid
        INTEGER :: group
        ...
        ELSE
          ...
          msglen = SIZE(msg_sour)
        END IF
        IF( msglen*16 > mp_msgsiz_max ) CALL mp_stop(8925)
#if defined(__MPI)
        CALL MPI_BARRIER(group, IERR)
        IF (ierr/=0) CALL mp_stop(8140)
#endif
...
!------------------------------------------------------------------------------!
      ...
      SUBROUTINE mp_sum_r4d(msg,gid)
        ...
        INTEGER, OPTIONAL, INTENT(IN) :: gid
        INTEGER :: group
        INTEGER :: msglen, m1, m2, m3, m4, ierr
        REAL (dbl), ALLOCATABLE :: res(:,:,:,:)
#if defined(__MPI)
        group = mpi_comm_world
        IF( PRESENT( gid ) ) group = gid
