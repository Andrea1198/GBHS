...
!
  ...
  !
  PUBLIC  :: dft, dft_shortname
  !
  ! PRIVATE variables defining the DFT functional
  !
  PRIVATE :: iexch, icorr, igcx, igcc, inlc
...
CONTAINS
  ...
  subroutine set_dft_from_name( dft_ )
    ...
    !
    USE parser_module,      ONLY: matches, capital
    !
    implicit none
    ! input
    character(len=*)               :: dft_
...
!
           ...
           fz = ( (1.0_DP + zeta_eff) **p43 + (1.0_DP - zeta_eff) **p43 - 2.0_DP) &
                / (2.0_DP**p43 - 2.0_DP)
           fz1 = p43 * ( (1.0_DP + zeta_eff) **third- (1.0_DP - zeta_eff) **third) &
                / (2.0_DP**p43 - 2.0_DP)
           fz2 = p49 * ( (1.0_DP + zeta_eff) **m23 + (1.0_DP - zeta_eff) **m23) &
                / (2.0_DP**p43 - 2.0_DP)
           aa = dmcu + fz * (dmcp - dmcu)
           bb = 2.0_DP * fz1 * (vcp - vcu - (ecp - ecu) ) / rhotot
           cc = fz2 * (ecp - ecu) / rhotot
           dmuxc_uu = dmuxc_uu + aa + (1.0_DP - zeta_eff) * bb + (1.0_DP - zeta_eff)**2 * cc
           dmuxc_du = dmuxc_du + aa + ( -zeta_eff) * bb   + (zeta_eff**2 -1.0_DP) * cc
           dmuxc_ud = dmuxc_du
           dmuxc_dd = dmuxc_dd + aa - (1.0_DP + zeta_eff) * bb + (1.0_DP + zeta_eff)**2 * cc
