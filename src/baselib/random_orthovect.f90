...
!
!*******************************
MODULE random_orthovect_module
   !*******************************
   !
   IMPLICIT NONE
   PRIVATE
   !
   INTERFACE random_orthovect
       MODULE PROCEDURE random_orthovect_r
       MODULE PROCEDURE random_orthovect_c
   END INTERFACE
   !
   INTERFACE random_vect
       MODULE PROCEDURE random_vect_r
       MODULE PROCEDURE random_vect_c
   END INTERFACE
   !
   PUBLIC :: random_vect
   PUBLIC :: random_orthovect
   !
CONTAINS
!
!********************************************************
   SUBROUTINE random_orthovect_r( n, ndim, ldx, vect, proj)
   !********************************************************
   !
   ! This subroutines generates N randomized orthonormal vectors 
   ! with dimension NDIM. Vectors are orthonormalized throught a
   ! simple Gram Schmidt procedure.
   ! The projector PROJ is acted on the vectors if present.
   ! The rndm() function is used as random number generator.
   !
   !
   USE kinds,                 ONLY : dbl
   USE constants,             ONLY : ZERO, ONE, EPS_m6
   ...
   REAL(dbl),           INTENT(OUT) :: vect(ldx, n)
   REAL(dbl), OPTIONAL, INTENT(IN)  :: proj(ndim,ndim)
...
!
   ...
   DO i = 1, n
       ...
       DO WHILE( ncount < nmax .AND. .NOT. found )
           ...
           CALL random_vect_r( ndim, vect(1,i) ) 

           !
           ! apply the projector, if present
           !
           IF ( PRESENT( proj ) ) THEN
               !
               CALL mat_mul( vect(:,i:i), proj, "N", vect(:,i:i), "N", ndim, 1, ndim)
               !
           ENDIF
           !
           ...
           !
           CALL mat_mul( aux, vect(1:ndim, 1:i), 'C', vect(1:ndim,i:i), 'N', i, 1, ndim )
           !
           ...
           ! vectors are then linearly indep
           !
           norm = ZERO
           DO j=1, i-1
              norm = norm + REAL( aux(j,1) * aux(j,1) )
           ENDDO
           IF ( norm < REAL (aux(i,1)) - EPS_m6 ) THEN
                ...
                norm = ZERO
                DO j=1, ndim
                   norm = norm + REAL( vect(j,i) * vect(j,i) )
                ENDDO
                norm = ONE / SQRT(norm)
                vect (:,i) = vect(:,i) * norm
           ENDIF
...
END SUBROUTINE random_orthovect_r


!********************************************************
   SUBROUTINE random_vect_r(ndim, z)
   !********************************************************
   !
   ...
   RETURN   
END SUBROUTINE random_vect_r

!
!********************************************************
   SUBROUTINE random_orthovect_c( n, ndim, ldx, vect, proj)
   !********************************************************
   !
   ! This subroutines generates N randomized orthonormal vectors 
   ! with dimension NDIM. Vectors are orthonormalized throught a
   ...
   COMPLEX(dbl), OPTIONAL, INTENT(IN)  :: proj(ndim,ndim)
...
!
   ...
   DO i = 1, n
       ...
       DO WHILE( ncount < nmax .AND. .NOT. found )
           ...
           !
           ! generate a new non-null vector
           CALL random_vect_c( ndim, vect(1,i) ) 
...
END SUBROUTINE random_orthovect_c


!********************************************************
   SUBROUTINE random_vect_c(ndim, z)
   !********************************************************
   !
   ! generates a non-null random vector
   !
   ...
   IF ( .NOT. found ) CALL errore('random_vect','maximum number of attempts reached',nmax)
   RETURN   
END SUBROUTINE random_vect_c

END MODULE random_orthovect_module
