...
!*********************************************
   ...
   USE mp,                ONLY : mp_sum
# if defined _OPENMP
   USE openmp_m,          ONLY : omp_get_thread_num
# endif
   !
   IMPLICIT NONE
   PRIVATE
...
#if defined _OPENMP
      if ( .not. (omp_get_thread_num() .eq. 0) ) return
# endif
      !
...
!**********************************************************
SUBROUTINE timing_ext( subname, opr)
   !**********************************************************
   USE timing_module
   IMPLICIT NONE
   !
   CHARACTER(*), INTENT(IN) :: subname
   CHARACTER(*), INTENT(IN) :: opr
   !
   CALL timing( subname, opr)
   !
END SUBROUTINE timing_ext
