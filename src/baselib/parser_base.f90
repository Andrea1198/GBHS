...
! SUBROUTINE parser_list(str,nitem[,fs][,list],[,ierr])
...
!**********************************************************
   SUBROUTINE parser_list(str,nitem,fs,list)
   !**********************************************************
   IMPLICIT NONE
   CHARACTER(*),           INTENT(IN)    :: str        
   INTEGER,                INTENT(OUT)   :: nitem
   CHARACTER,    OPTIONAL, INTENT(IN)    :: fs
   CHARACTER(*), OPTIONAL, INTENT(OUT)   :: list(*)
   ...
   DO i=1,length
       !
       IF ( str_(i:i) /= fs_ ) THEN
           ind=ind+1
           str_tmp(ind:ind)=str_(i:i)
       ELSE
           IF ( LEN_TRIM(str_tmp(1:ind))> 0 ) THEN 
               nitem=nitem+1
               IF ( PRESENT(list)) list(nitem)=TRIM(ADJUSTL(str_tmp))
               str_tmp=" "
               ind=0
           ELSE
               str_tmp=" "
