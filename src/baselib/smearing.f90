...
!**********************************************************
   SUBROUTINE smearing_init( smearing_type_, delta_, delta_ratio_, xmax_, smearing_type_null_ )
   !**********************************************************
   IMPLICIT NONE
       !
       CHARACTER(*),           INTENT(IN) :: smearing_type_
       REAL(dbl),              INTENT(IN) :: delta_
       REAL(dbl),    OPTIONAL, INTENT(IN) :: delta_ratio_
       REAL(dbl),    OPTIONAL, INTENT(IN) :: xmax_
       CHARACTER(*), OPTIONAL, INTENT(IN) :: smearing_type_null_
       !
       ...
       !
       IF ( PRESENT( smearing_type_null_) )  smearing_type_null = TRIM(smearing_type_null)
       IF ( PRESENT( delta_ratio_) )                delta_ratio = delta_ratio_
...
!**********************************************************
   FUNCTION smearing_gfunc( x, delta )
   !**********************************************************
   !
   ! evaluate the GF function with a generic smearing
   !
   IMPLICIT NONE
       !
       REAL(dbl)    :: x, delta
       COMPLEX(dbl) :: smearing_gfunc 
       !
       !CHARACTER(14)  :: subname="smearing_gfunc"
       REAL(dbl)      :: wx
       COMPLEX(dbl)   :: g1, g2, gs
       INTEGER        :: ix
       !
       !
       IF ( TRIM(smearing_type) == "lorentzian" ) THEN
           !
           smearing_gfunc = CONE / ( delta * ( x + CI ) )
           !
           RETURN
       ENDIF
       !
       CALL locate( xgrid, nx, x, ix )
       !
       ...
       IF ( ix == 0 .OR. ix == nx ) THEN
           ...
           !
           gs = REAL( CONE / ( delta * ( x + CI) ) , KIND=dbl )
           !

       ELSE
           !
           g1 = g_smear(ix) / delta
           g2 = g_smear(ix+1) / delta
           !
